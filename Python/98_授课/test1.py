...
starting_stack_size= 103
max_stable_height= 20
partition = 5
globalData = []

def myPartition(stack_size):
  myData = []
  result = int(stack_size/partition)
  for i in range(partition):
      myData.append(result)

  mod = stack_size%partition
  if mod > 0:
    for j in range(mod):
      myData[j] += 1
  
  for number in myData:
    if number <= max_stable_height:
      globalData.append(number)
    else:
      myPartition(number)
...
if starting_stack_size <= partition:
  ...
  myPartition(starting_stack_size)
