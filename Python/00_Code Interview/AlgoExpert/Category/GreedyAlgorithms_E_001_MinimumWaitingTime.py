...
the duration of the first query; if a query is executed third, then its waiting time 
...
waiting time if the queries were executed in the orders of [5,1,4] would be:
0+5+ (5+1) = 11. The first query of duration 5 would be executed imme4diately, so its 
waiting time would be 0, the second query of duration 1 would have to wait 5 seconds 
(the duration of the first query) to be executed, and the last query would have to 
...
def minimumWaitingTime (queries):
	...
	for i in range(1, len(queries)):
		currWaiting += queries[i-1]
		waitingSum += currWaiting
